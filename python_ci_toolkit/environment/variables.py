...
ENVIRONMENTS_WITHOUT_MULTILINE_SUPPORT = [
    CiEnvironmentType.BitbucketPipelines,
]
"""List of CI environments that do not support multiline environment variables."""
...
def retrieve_environment_variable(variable_name: str, usage_explanation: str = None,
                                  fallback_value: Callable[[], str | None] | str = None,
    ...
    Notes:
        ...
        If current CI environment supports multiline environment variables, this function will return variable's original value without modifying it.
    ...
    Args:
        ...
        fallback_value: Value to be returned if the given environment variable is not set.
        ...
        newline_substitution_character: (only applies to CI environments without multiline env vars support)
    ...
    if is_environment_variable_set(variable_name):
        ...
        if ci_environment_type in ENVIRONMENTS_WITHOUT_MULTILINE_SUPPORT:
    ...
    fallback_is_provided = fallback_value is not None
    if fallback_is_provided:
        ...
        fallback_is_a_callable = callable(fallback_value)
        ...
        # if fallback value is not a string and not empty, raise a TypeError
            ...
            if fallback_is_a_callable:
                message = (f"Fallback getter function '{fallback_value}' returned value of type {type(fallback_value)}, which is neither a string nor None.\n"
                ...
                message = (f"Fallback value '{fallback_value}' is of type {type(fallback_value)}, which is neither a string nor None.\n"
