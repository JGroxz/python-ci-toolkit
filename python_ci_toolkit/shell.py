...
def _run_shell_command(
    ...
    command_display_string = command if isinstance(command, str) else shlex.join(command)

    # print header if using pretty output
    if (not quiet) and (not raw_output):
        header = (
            Text("Running shell command:", style=SHELL_OUTPUT_PREFIX_STYLE)
            + " "
            + Text(command_display_string, style=SHELL_OUTPUT_COMMAND_STYLE)
        )
