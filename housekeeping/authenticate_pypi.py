...
def cli():
    ...
    # PIP
    run_shell_command(f"pip config set global.extra-index-url https://{pypi_user}:{pypi_token}@{pypi_url}", use_wsl_on_windows=False)

    # Poetry
    run_shell_command(f"poetry config http-basic.{ENV_AWS_PYPI_REPO_NAME} {pypi_user} {pypi_token}", use_wsl_on_windows=False)
