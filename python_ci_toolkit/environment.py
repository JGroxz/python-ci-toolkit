...
ci_temp_files_directory_relative: Path = ci_files_directory_relative / "temp/"
"""
...
ci_temp_files_directory: Path = ci_project_root / ci_temp_files_directory_relative
"""
...
def ensure_ci_temp_files_directory_exists() -> None:
    """
    Ensures that the directory for temporary CI files exists in the current CI project and is included in .gitignore.
    """
    ...
    try:
        ...
        # current CI project root is not a Git repository; in this case we cannot be sure where the toolkit is being run,
        # and refrain from creating the temporary files directory to avoid cluttering random directories with temp files;
        # this will lead CI code relying on temporary files to fail, but it is an intended behavior outside of Git repos
        return
