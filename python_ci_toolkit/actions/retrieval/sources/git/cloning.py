...
def get_remote_action_repo() -> str | None:
    """
    Returns:
        None if no remote action repository is configured.
        Otherwise, URL of the remote Git repository that contains actions.
    """
    return _get_configured_action_repo_url()


def require_remote_action_repo() -> str:
    """
...
@log_execution_time
def retrieve_action_repo(git_repo_url: str) -> Path:
    """
...
@log_execution_time
def retrieve_ci_action_script_from_git(git_repo_url: str, action_name: str, action_version: str = None) -> Path:
    """
    Retrieves Python CI script file with the given name from a Git repository.

    Notes:
        Action scripts must be located in a configured remote action Git repository.
        ...
        PyCI does not manage Git authentication. The configured repository URL must be fetchable by
        Git in the current environment.
    ...
    # retrieve repo
    cloned_actions_directory = retrieve_action_repo(git_repo_url)
