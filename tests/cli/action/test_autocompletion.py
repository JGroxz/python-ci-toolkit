def test_autocompletion_list_available_actions():
    ...
    def test():
        all_actions_metadata = list_available_actions()
    ...
    action_cli_module._ACTION_AUTOCOMPLETE_REMOTE_CLONE_CACHE_TIMEOUT = 0  # <- to avoid caching
    test()

    # run again to verify that caching does not affect the result
    action_cli_module._ACTION_AUTOCOMPLETE_REMOTE_CLONE_CACHE_TIMEOUT = 1000  # <- to enable caching
    test()
